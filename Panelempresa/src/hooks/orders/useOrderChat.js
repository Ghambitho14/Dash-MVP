...
/**
 * Hook para gestionar chat de pedidos - PanelEmpresa
 * Chat entre empresa y repartidor sobre un pedido específico
 */
export function useOrderChat(order, currentUser) {
	...
	// Inicializar chat cuando hay pedido con repartidor asignado
	useEffect(() => {
		if (!order || !order.driverId || !currentUser) {
			setChat(null);
			setMessages([]);
			return;
		}
		...
		initializeChat();
	}, [order?._dbId, order?.driverId, currentUser]);
	...
	const loadMessages = useCallback(async (chatId, showLoading = true) => {
		...
		try {
			...
			if (currentUser) {
				await markOrderChatMessagesAsRead(chatId, getDbId(currentUser), 'company_user');
			}
		} catch (err) {
			...
			.on(
				...
				(payload) => {
					...
					if (newMessage.sender_id !== userId && newMessage.sender_type !== 'company_user') {
						markOrderChatMessagesAsRead(chatId, userId, 'company_user').catch(err => {
